...
export default function AuthForm({ type, title }: AuthFormProps) {
  ...
  const showToastMessage = (message: string) => {
    ...
    setShowToast(true);
    setTimeout(() => setShowToast(false), 13000);
  };
    ...
    <>
      ...
      >
        <Toast
          ...
          delay={17000}
          autohide
          className={`toast ${styles.fullWidthToast}`}
          bg="danger"
          style={{ width: '100%', position: 'fixed', left: 0, zIndex: 1050 }}
        >
          <Toast.Body style={{ color: 'white' }}>{loginError}</Toast.Body>
        </Toast>
      </div>
    </>
  );
